...
class MembersOfParliament(Base):
    ...
    expenses = relationship('Expense', back_populates='member')
    ...
    @classmethod
    def get_members_by_caucus(cls, session, caucus_name):
        """Retrieve all members from a specific caucus."""
        return session.query(cls).filter_by(caucus=caucus_name).all()
    
    @classmethod
    def get_members_by_constituency(cls, session, constituency_name):
    ...
    @classmethod
    def get_members_by_last_name(cls, session, mp_last_name):
        """Retrieves member(s) by their last name."""
        return session.query(cls).filter_by(last_name=mp_last_name).all()

    @classmethod
    def get_members_by_first_name(cls, session, mp_first_name):
